# Changelog

## Phase 4 - React, Storybook and testing tooling

- Configure private React/TypeScript ESM and CSS builds using the existing token package.
- Add Storybook React/Vite, Light/Dark controls, browser interaction tests and axe accessibility checks.
- Add locked dependencies, CI verification and concise development guidance. No production components, new tokens or publishing.

## Token package 0.2.0 - Phase 3

- Capture 292 current Figma variables, 16 text styles and 14 effect styles with source IDs, aliases, modes and raw snapshot hashes.
- Generate DTCG-compatible tokens, Light/Dark semantic and chart themes, CSS custom properties and typed structured data.
- Add dependency-free validation and regression tests. Preserve the five existing component spacing additions separately from the base scale.
- Keep React, Storybook and publication out of scope.

## Version 0.1.0 — Repository Foundation

2026-09-24

- Repository initialized locally.
- Documentation structure added.
- Token workspace prepared.
- React workspace prepared.
- Storybook workspace prepared.
- Accessibility documentation structure added.
- Initial decision records and asset provenance guidance added.

This version is a repository milestone, not a published component release. No components, token values, stories, runtime dependencies or CI workflows are implemented.
